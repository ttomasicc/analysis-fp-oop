 ...
 */
function loadMeteoData(filePath: string): any {
    try {
 ...
 */
const formatCSV = (data: string): Array<Array<string>> =>
    data
        .split('\n')
        .filter(Boolean)
        .map(line => line.split(';'))
 ...
 */
function groupBy(data: Array<any>, attrGetterFn: Function): object {
 ...
 */
function groupMeteoData(meteoData: string): object {
 ...
 */
function processMeteoData(meteoData: object): object {
 ...
 */
function getDayStatistics(day: Array<Array<string>>): object {
    const statistics = {
        minTemp: Infinity,
        maxTemp: -Infinity,
        avgTemp: NaN,
